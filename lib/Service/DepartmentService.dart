import 'dart:convert';

import 'package:http/http.dart' as http;

class Departmentservice {
  String baseUrl = "http://147.93.97.78:5030/";
  Future<List<Map<String, dynamic>>> fetchAllDepartmentData() async {
    List<Map<String, dynamic>> responseDataTable = [];

    try {
      final response = await http.get(Uri.parse("${baseUrl}department/"));
      if (response.statusCode == 200) {
        
        final List<dynamic> data = jsonDecode(response.body);
        responseDataTable = data.map((data) {
          return {
            "No": data['id'],
            "Title": data['name'],
            "Leader Name": data['department_leader'],
            "contact": data['contact'],
            "Action": 'Action',
          };
        }).toList();
      } else {
        print("Error fetching data. Status code: ${response.statusCode}");
      }
    } catch (e) {
      print("Error fetching data: $e");
    }

    return [
      {
        "header": ['No', 'Title', 'Leader Name', 'contact', 'Action'],
        "dataType": ['Text', 'Text', 'Text', 'Text', 'Action'],
        "data": responseDataTable
      }
    ];
  }

  Future<bool> createDepartment({
    required String name,
    required String departmentLeader,
    required String contact,
    required String location,
    required String createdBy,
    required String description,
    required String city,
    required String country,
    required String pincode,
  }) async {
    final url = Uri.parse("${baseUrl}department/");
    final payload = {
      "name": name,
      "department_leader": departmentLeader,
      "contact": contact,
      "created_by": createdBy,
      "location": location,
      "description": description,
      "city": city,
      "country": country,
      "pincode": pincode,
    };

    try {
      final response = await http.post(
        url,
        headers: {
          'Content-Type': 'application/json',
        },
        body: jsonEncode(payload),
      );

      if (response.statusCode == 200 || response.statusCode == 201) {
        return true;
      } else {
        print(
            'Failed to create department. Status code: ${response.statusCode}, Response: ${response.body}');
        return false;
      }
    } catch (e) {
      print('Error creating department: $e');
      return false;
    }
  }
}
