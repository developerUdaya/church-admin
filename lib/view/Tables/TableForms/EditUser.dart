...
class MyApp2 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const EditUser(userId: "183",),
    );
  }
}
...
class EditUser extends StatefulWidget {
  ...
  const EditUser({super.key, this.userId,});
...
class _EditUserState extends State<EditUser> {
  ...
  Future<void> fetchUserDetails(String userId) async {
    final String url = "http://147.93.97.78:5030/user_with_other_details/$userId/";
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _contactNumberController.text = data['user']['contact_number'] ?? '';
          _dobController.text = data['user']['dob'] ?? '';
          _joinedDateController.text = data['user']['joined_date'] ?? '';
          _additionalMobileController.text =
              data['user']['additional_mobile_number'] ?? '';
          _selectedPrefix = data['user']['prefix'] ?? 'Mr.';
          _selectedGender = data['additional_info']['gender'] ?? 'male';
          _selectedMaritalStatus =
              data['additional_info']['marital_status'] ?? 'single';
          _baptismDateController.text =
              data['additional_info']['baptism_date'] ?? '';
          _previousChurchController.text =
              data['additional_info']['previous_church'] ?? '';
          _aadharNumberController.text =
              data['additional_info']['aadhar_number'] ?? '';
          _relationWithFamilyController.text =
              data['additional_info']['relation_with_family'] ?? '';
          _hobbiesController.text = data['additional_info']['hobbies'] ?? '';
          _selectedHouseType = data['additional_info']['house_type'] ?? 'owned';
          _selectedAttendingTime =
              data['additional_info']['attending_time'] ?? 'morning';
          _selectedNationality =
              data['additional_info']['nationality'] ?? 'indian';
          _selectedRelationToFamily =
              data['additional_info']['relation_to_family'] ?? 1;
          _selectedZone = data['additional_info']['zone'] ?? 1;
          _addressLine1Controller.text =
              data['addresses'][0]['address_line_1'] ?? '';
          _addressLine2Controller.text =
              data['addresses'][0]['address_line_2'] ?? '';
          _cityController.text = data['addresses'][0]['city'] ?? '';
          _stateController.text = data['addresses'][0]['state'] ?? '';
          _postalCodeController.text =
              data['addresses'][0]['postal_code'] ?? '';
          _countryController.text = data['addresses'][0]['country'] ?? '';
          _landmarkController.text = data['addresses'][0]['landmark'] ?? '';
          _professionNameController.text =
              data['professional_details']['profession_name'] ?? '';
          _qualificationController.text =
              data['professional_details']['qualification'] ?? '';
          _designationController.text =
              data['professional_details']['designation'] ?? '';
          _companyNameController.text =
              data['professional_details']['company_name'] ?? '';
          _departmentController.text =
              data['professional_details']['department'] ?? '';
          _yearsOfExperienceController.text =
              data['professional_details']['years_of_experience'].toString();
          _selectedProfessionCategory =
              data['professional_details']['category'] ?? 'it';
          _selectedEmploymentType =
              data['professional_details']['type'] ?? 'self_employed';
          _lastDonatedDateController.text =
              data['blood_group']['last_donated_date'] ?? '';
          _selectedBloodGroup =
              data['blood_group']['blood_group_name'] ?? 'A-';
        });
      } else {
  ...
  Future<void> _pickImageFromGallery() async {
    ...
    try {
     ...
     // debug// print('Error in _pickImageFromGallery: $e');
    }
  }
  ...
  Future<void> _pickImageFromCamera() async {
    try {
     ...
     // debug// print('Error in _pickImageFromCamera: $e');
    }
  }
